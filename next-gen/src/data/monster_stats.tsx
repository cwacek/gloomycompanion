...
interface MonsterStatsCollection {
  monsters: { [key: string]: { level: MonsterStats[] } };
  bosses: { [key: string]: { level: BossAttrs[] } };
}

export const MONSTER_STATS: MonsterStatsCollection = {
  "monsters": {
    "Ancient Artillery": {
      "level": [
        {
...
export function getBossStats(name: string, level: number): BossAttrs {
  let stats = MONSTER_STATS.bosses[name].level.find((def) => def.level === level)
  if (!stats) {
    throw new Error(`Failed to find level ${level} ${name}`)
  }
  return stats
